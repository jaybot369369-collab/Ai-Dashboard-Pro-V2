...
const R2 = (() => {
  ...
  /* Jay's existing uploader, carried over from the old dashboard so her
     screenshots keep going to the same bucket. She can change it in Settings.
     Only fills in if nothing is set yet — never overwrites her own value. */
  // Blanked for the design copy — the real build seeds the owner's uploader here.
  const DEFAULT_WORKER = '';
  (function seedWorker() {
    if (!get(KEYS.workerUrl)) setS(KEYS.workerUrl, DEFAULT_WORKER);
    if (get(KEYS.enabled) === '') setS(KEYS.enabled, '1');
  })();
