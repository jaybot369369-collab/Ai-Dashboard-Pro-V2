...
const OrderBookTab = (() => {
  ...
  /* Signal colours are read from the theme — light grey up, dark grey down. */
  let C_BULL = '#767e8e', C_BEAR = '#1b1f26', C_WARN = '#a86a00', C_FLAT = '#9aa0aa';
  function _syncSignalColours() {
    const cs = getComputedStyle(document.documentElement);
    const v = (n, f) => (cs.getPropertyValue(n) || f).trim();
    C_BULL = v('--good', C_BULL); C_BEAR = v('--bad', C_BEAR);
    C_WARN = v('--warn', C_WARN); C_FLAT = v('--text-3', C_FLAT);
  }
  _syncSignalColours();
  new MutationObserver(_syncSignalColours)
  ...
  function _verdict(tone, label, sub) {
    ...
    const bg = `color-mix(in srgb,${c} 13%,var(--surface))`;
  ...
  // Signed bar chart with a zero line (funding). pos→dark, neg→light.
  ...
  function _sigETF() {
    ...
    if (s7 < 0)             return { tone: 'warn', lbl: 'Flows rolling over',   sub: `30D still ${f(s30)}, but last 7D turned ${f(s7)}` };
  ...
  function _paintLiqHeatmap() {
    ...
    if (srcEl) {
      ...
      Object.assign(srcEl.style, {
        background: live ? 'color-mix(in srgb,var(--good) 18%,transparent)' : 'color-mix(in srgb,var(--warn) 16%,transparent)',
        color:      live ? 'var(--good-ink)' : 'var(--warn)',
        border:     '1px solid ' + (live ? 'color-mix(in srgb,var(--good) 45%,transparent)' : 'color-mix(in srgb,var(--warn) 45%,transparent)'),
      });
    }
    ...
    if (!all.length || !current) {
      chartEl.innerHTML = `<div style="padding:60px 0;text-align:center;color:var(--muted);font-size:.82rem">Warming up — zones appear within ~1 min</div>`;
      return;
    }
    ...
    s += `<rect x="${PL}" y="${PT}" width="${cW}" height="${Math.max(0, cy - PT)}" fill="rgba(255,255,255,0.05)"/>`;
    s += `<rect x="${PL}" y="${cy}" width="${cW}" height="${Math.max(0, PT + cH - cy)}" fill="rgba(0,0,0,0.22)"/>`;
    for (let i = 1; i < 5; i++) {
      const gy = PT + (cH / 5) * i;
    ...
    if (klines && klines.length) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const bull = k.c >= k.o, col = bull ? 'rgba(228,234,244,0.88)' : 'rgba(122,131,146,0.92)';
    ...
    [...all].sort((a, b) => b.liq_usd - a.liq_usd).forEach(r => {
      ...
      usedY.add(y);
      const col = r.price > current ? 'rgba(228,234,244,0.95)' : 'rgba(132,141,156,0.95)';
  ...
  function _paintStatus() {
    ...
    const map = { live: ['● live', 'var(--good-ink)'], connecting: ['○ connecting…', 'var(--muted)'], reconnecting: ['◌ reconnecting…', 'var(--warn)'], idle: ['○ idle', 'var(--muted)'] };
  ...
  function _etfGuideHTML() {
    const rows = [
      ...
      ['', 'LONG — Swing mode',
       'Verdict is INFLOW — 7D and 30D both positive, inflow streak building',
      ...
      ['', 'SHORT — Swing mode',
       'Verdict is OUTFLOW — 7D and 30D both negative, outflow streak building',
    ...
    return `<div class="ob-card ob-setup-guide">
      ...
      <div class="ob-sg-foot">⏱ Timing truth: flows publish ONCE per day, after the US close — this is a <b>bias filter, not an entry trigger</b>. Scalp = direction filter for today. Swing = regime filter for the week. One giant day (±$1B+) outweighs five small ones. Transition verdicts (rolling over / easing) — size down or wait. Entries always come from your ICT setup.</div>
    </div>`;
  }
      ...
      <div id="ob-root" class="ob-wrap">
        ...
        <div class="ob-card">
          <div class="ob-card-h">
            ...
            <span class="ob-sub">estimated positions at risk · blue = largest cluster · not exchange-confirmed</span>
  ...
  function _matrixHTML(key) {
    ...
    if (key === 'funding') {
      ...
      tbl = _mtable(['Price', 'OI', 'What it means', 'Scenario'], [
        ...
        [_UP, _DN, 'Short covering only',    _WARN('Weak rally — up-move losing steam')],
        [_DN, _UP, 'New shorts entering',    _BEAR('Bearish — real conviction selling')],
        [_DN, _DN, 'Longs bailing out',      _WARN('Weak selloff — downtrend exhausting')],
      ]);
    } else if (key === 'cvd') {
      title = 'Reading CVD = Price + taker flow together';
      tbl = _mtable(['Price', 'CVD bars', 'What it means', 'Scenario'], [
        [_UP, _BULL('Up'), 'Buyers driving the move',          _BULL('Bullish — confirmed buying')],
        [_UP, _BEAR('Down'),   'Sellers hitting, price still up',  _WARN('Bull trap — divergence, fade it')],
        [_DN, _BEAR('Down'),   'Sellers driving the move',         _BEAR('Bearish — confirmed selling')],
        [_DN, _BULL('Up'), 'Buyers absorbing the dump',        _BULL('Absorption — reversal UP brewing')],
      ]);
    } else if (key === 'liq') {
      title = 'Reading the heatmap (clusters are price magnets)';
      tbl = _mtable(['Where the big cluster sits', 'What it means', 'Scenario'], [
        ...
        [_BULL('Big cluster ABOVE price'), 'Short stops stacked over price',  _BEAR('Magnet up — expect a sweep then reject. SHORT the sweep, not before')],
        ['Equal both sides', 'No clear magnet',                              'Range — wait until one side gets swept'],
        [_WARN('Cluster on YOUR stop'), 'Your stop is the target',           _WARN('Move it — high odds it gets hunted')],
      ]);
    } else if (key === 'etf') {
      ...
      tbl = _mtable(['Price', 'ETF flows', 'What it means', 'Scenario'], [
        ...
        [_UP, _BEAR('Outflows'), 'Price up but institutions selling',    _WARN('Rally on thin ice — trail stops, don’t chase')],
      ...
      tbl = _mtable(['Wall', 'What it means', 'Scenario'], [
        ...
        [_BEAR('Ask wall at your target'), 'Overhead supply at your TP',   _WARN('Bank profit just before it')],
      ]);
    } else return '';
    ...
    <div class="ob-card ob-guide collapsed">
      ...
      <div class="ob-guide-body">
        ...
        <div class="ob-guide-panels">
          <div class="ob-guide-prow"><span class="ob-gp-icon"></span><div><strong>Liq heatmap</strong> — shows where estimated liq clusters sit. <em>Flush zones</em> (below price, dark tint) = long-squeeze targets. <em>Squeeze zones</em> (above price, light tint) = short-squeeze targets. These are price magnets — smart money often hunts the nearest cluster before reversing. Brightest blue = largest concentration. Your stop must not sit inside a dense cluster.</div></div>
          ...
          <div class="ob-guide-prow"><span class="ob-gp-icon"></span><div><strong>CVD bars</strong> — Cumulative Volume Delta binned into your TF candles. Up bar = net taker buying that candle; down bar = net selling. The pale line is the running CVD total. Multiple down bars while price holds a bid-side OB = sellers are aggressive but price won't move — that's <em>absorption</em>, a bullish sign. Multiple up bars while price falls = potential bull trap.</div></div>
        ...
        <div class="ob-scene ob-scene-bull">
          ...
          <div class="ob-scene-grid">
            ...
            <div class="ob-sc-state">3+ consecutive <strong>up bars</strong>, running line sloping up</div>
        ...
        <div class="ob-scene ob-scene-bear">
          ...
          <div class="ob-scene-grid">
            ...
            <div class="ob-sc-panel">CVD</div>
            <div class="ob-sc-state">3+ consecutive <strong>down bars</strong>, running line rolling over</div>
        ...
        <div class="ob-scene ob-scene-neutral">
          ...
          <div class="ob-scene-grid">
            ...
            <div class="ob-sc-panel">CVD</div>
            <div class="ob-sc-state">Alternating up/down bars, flat running line</div>
...
/* ── Page wrapper for the Dashboard 3 router ──────────────────────────
   OrderBookTab above is the full Level 2 module carried over from the old
   dashboard, untouched: liquidation heatmap, funding, open interest, CVD,
   order-book walls, ETF flows, the confluence card and every how-to-read
   panel. This just gives it the shape the router expects and makes sure
   its websockets are shut down when Jay leaves the page. */
const Level2Page = {
  render: () => OrderBookTab.render(),
  leave:  () => { try { OrderBookTab.teardown(); } catch (e) {} },
  title:  'Level 2',
  sub:    () => 'Order flow, funding, open interest and where the stops sit',
};
