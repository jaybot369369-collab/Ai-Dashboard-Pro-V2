...
const TendenciesTab = (() => {
  ...
  /* Chart.js paints into a canvas, so the theme has to be resolved to real
     values rather than handed a var() reference. */
  function _tc(name, fallback) {
    const v = getComputedStyle(document.documentElement).getPropertyValue(name);
    return (v && v.trim()) || fallback;
  }

  function _heatColor(pl, n, maxAbs) {
    if (!n) return 'color-mix(in srgb,var(--text-3) 6%,transparent)';
    if (pl === 0) return 'color-mix(in srgb,var(--text-3) 16%,transparent)';
    const ratio = Math.min(1, Math.abs(pl) / maxAbs);
    const pct = Math.round(16 + 64 * ratio);
    return `color-mix(in srgb,var(${pl > 0 ? '--good' : '--bad'}) ${pct}%,transparent)`;
  }
  ...
  function _heatmapCard() {
    ...
    if (!total) {
      return `<div id="tendHeatmap" class="card" style="padding:16px;margin-bottom:16px">
        <div style="font-size:.7rem;text-transform:uppercase;letter-spacing:.08em;color:var(--text-3);margin-bottom:2px">Day × Time P&amp;L Map</div>
    ...
    return `<div id="tendHeatmap" class="card" style="padding:16px;margin-bottom:16px">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap;margin-bottom:10px">
        <div>
          <div style="font-size:.7rem;text-transform:uppercase;letter-spacing:.08em;color:var(--text-3);margin-bottom:2px">Day × Time P&amp;L Map</div>
          <div style="font-size:.85rem;font-weight:600;color:var(--text)">When your edge shows up — light cells make money, dark cells lose it</div>
        </div>
        ${modeBtns}
      </div>
  ...
  function _hmCell(day, col) {
    ...
    const rows = list.map(t => {
      ...
      return `<tr onclick="TradeView.open('${esc(t.id)}')" title="Open trade">
        ...
        <td><span style="color:${t.direction === 'Long' ? 'var(--good-ink)' : 'var(--bad)'};font-weight:600">${esc(t.direction || '—')}</span></td>
        <td class="hm-td-setup">${esc(setups)}</td>
        <td>${t.rMultiple !== undefined && t.rMultiple !== '' && t.rMultiple !== null ? esc(t.rMultiple) + 'R' : '—'}</td>
        <td style="text-align:right;font-weight:700;color:${tpl >= 0 ? 'var(--good-ink)' : 'var(--bad)'}">${fmtPL(tpl)}</td>
      </tr>`;
    }).join('');
    ...
    el.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:8px">
        <span><b>${esc(day)} · ${esc(colLabel)}</b> — ${n} trade${n === 1 ? '' : 's'} · ${Math.round(wins / n * 100)}% WR · ${avgR} avg ·
          <span style="color:${pl >= 0 ? 'var(--good-ink)' : 'var(--bad)'};font-weight:700">${fmtPL(pl)}</span></span>
  ...
  /* Writes a plain message on the canvas instead of leaving an empty box.
     An empty panel looks like "no data"; it usually means the chart library
     did not load. */
  function _chartMissing(canvas) {
    const ctx = canvas.getContext('2d');
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    ctx.fillStyle = _tc('--text-3', '#8b91a0');
    ctx.font = '12px system-ui';
    ctx.textAlign = 'center';
    ctx.fillText('Chart library did not load', canvas.width / 2, canvas.height / 2);
  }
  ...
  function _drawDOW(dowData) {
    ...
    if (!canvas) return;
    if (typeof Chart === 'undefined') { _chartMissing(canvas); return; }
    if (_dowChart) { _dowChart.destroy(); _dowChart = null; }
    const cGood = _tc('--good', '#767e8e'), cBad = _tc('--bad', '#1b1f26');
    const cDim = _tc('--text-3', '#8b91a0'), cLine = _tc('--line', 'rgba(0,0,0,.08)');
    const colors = dowData.map(d => d.pl >= 0 ? cGood : cBad);
    _dowChart = new Chart(canvas, {
      ...
      options: {
        ...
        scales: {
          x: { grid: { display: false }, ticks: { color: cDim, font: { size: 12 } } },
          y: { grid: { color: cLine }, ticks: { color: cDim, font: { size: 11 }, callback: v => '$' + v } }
        }
      }
    });
  }
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: hasData
            ? [_tc('--good', '#767e8e'), _tc('--bad', '#1b1f26')]
            : [_tc('--line', '#dcdde3'), _tc('--line', '#dcdde3')],
          borderWidth: 0,
          hoverOffset: 4,
        }]
      },
  ...
  function _renderAnalytics(trades) {
    ...
    const sessionRows = sessionData.length
      ? sessionData.map(s => {
          ...
          const color = s.pl >= 0 ? 'var(--good-ink)' : 'var(--bad)';
          return `<div style="margin-bottom:10px">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
              <span style="font-size:.82rem;font-weight:600;color:var(--text)">${esc(s.session)}</span>
              <div style="display:flex;gap:8px;align-items:center">
                <span style="font-size:.75rem;color:var(--text-3)">${s.wr}% WR</span>
                <span style="font-size:.82rem;font-weight:600;color:${s.pl>=0?'var(--good-ink)':'var(--bad)'}">${fmtPL(s.pl)}</span>
              </div>
            </div>
            <div style="height:5px;border-radius:3px;background:color-mix(in srgb,var(--text-3) 20%,transparent)">
      ...
      <div style="display:flex;flex-direction:column;gap:8px;min-width:120px">
        <div>
          <div style="display:flex;align-items:center;gap:6px;margin-bottom:2px">
            ...
            <span style="font-size:.8rem;color:var(--text-3)">Long</span>
          </div>
          <div style="font-size:.85rem;font-weight:700;color:var(--good-ink);margin-left:16px">${dirData.longs} trades</div>
          <div style="font-size:.75rem;color:var(--text-3);margin-left:16px">${fmtPL(dirData.lPL)}</div>
        </div>
        <div>
          <div style="display:flex;align-items:center;gap:6px;margin-bottom:2px">
            <div style="width:10px;height:10px;border-radius:2px;background:var(--bad);flex-shrink:0"></div>
            <span style="font-size:.8rem;color:var(--text-3)">Short</span>
          </div>
          <div style="font-size:.85rem;font-weight:700;color:var(--bad);margin-left:16px">${dirData.shorts} trades</div>
          <div style="font-size:.75rem;color:var(--text-3);margin-left:16px">${fmtPL(dirData.sPL)}</div>
        </div>
    ...
    return { html: `
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:24px">
        ...
        <div class="card" style="padding:16px">
          <div style="font-size:.7rem;text-transform:uppercase;letter-spacing:.08em;color:var(--text-3);margin-bottom:2px">P&amp;L by Day of Week</div>
        ...
        <!-- By Session -->
        <div class="card" style="padding:16px">
          <div style="font-size:.7rem;text-transform:uppercase;letter-spacing:.08em;color:var(--text-3);margin-bottom:2px">By Session</div>
        ...
        <!-- By Setup -->
        <div class="card" style="padding:16px">
          <div style="font-size:.7rem;text-transform:uppercase;letter-spacing:.08em;color:var(--text-3);margin-bottom:2px">By Setup</div>
        ...
        <!-- Direction Split -->
        <div class="card" style="padding:16px">
          <div style="font-size:.7rem;text-transform:uppercase;letter-spacing:.08em;color:var(--text-3);margin-bottom:2px">Direction Split</div>
          ...
          <div style="display:flex;align-items:center;gap:20px">
            <div style="position:relative;width:110px;height:110px;flex-shrink:0">
              ...
              <div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;pointer-events:none">
                ...
                <div style="font-size:.6rem;color:var(--text-3);text-transform:uppercase;letter-spacing:.06em">LONG BIAS</div>
              </div>
            </div>
            ${dirLegend}
          </div>
  ...
  function renderGrid(kind) {
    ...
    const accent = isMis ? 'var(--bad)' : 'var(--good-ink)';
    const accentBg = isMis ? 'color-mix(in srgb,var(--bad) 10%,transparent)' : 'color-mix(in srgb,var(--good) 14%,transparent)';
    const addLabel = isMis ? '＋ Add Mistake' : '＋ Add Strength';
    if (!items.length) {
      return `<div class="empty-state"><div class="empty-icon">${isMis?'':''}</div>
  ...
  function render() {
    ...
    content.innerHTML = `
      ...
      <div class="tend-wrap">
        <div class="tend-subnav">
          <button class="tend-sub-btn${_sub==='mistakes' ?' active mistakes':''}" data-sub="mistakes">Mistakes</button>
    ...
    /* Draw on a timer rather than the animation clock. A browser tab that is
       not actively painting can hold the animation callback back for a long
       time, and the two charts then never appear at all. */
    setTimeout(() => {
...
const TendenciesPage = {
  render: () => TendenciesTab.render(),
  title:  'Tendencies',
  sub:    () => 'What your trades keep saying about how you trade',
};
