...
const CryptoRadarTab = (() => {
  ...
  const LS_SIG_SERVER= 'jb_radar_sig_server';  // server mode — radar-1 farm bot fires (off browser clock)
  ...
  /* ── RSI scoring ─────────────────────────────────────────── */
  /* Wilder RSI. The old dashboard called ICTDetect.rsi(), but that file was not
     part of the bundle, so every coin scored null and every chart drew empty.
     Same maths, kept local; ICTDetect still wins if it ever loads. */
  function _rsiLocal(closes, period) {
  ...
  function _rsiForKlines(klines) {
    ...
    const v = (typeof ICTDetect !== 'undefined' && ICTDetect && typeof ICTDetect.rsi === 'function')
      ? ICTDetect.rsi(closes, 14)
      : _rsiLocal(closes, 14);
    return (v === null || v === undefined || !isFinite(v)) ? null : Math.round(v * 10) / 10;
  }
  ...
  function _radarSVG(rsiMap, sym) {
    ...
    const accent  = (cs.getPropertyValue('--text-3')  || '#8b91a0').trim();
    ...
    // Any spoke at RSI ≤ 10 → extreme oversold → blue center warning
    ...
    // Outer dark ring (RSI 70–100 annulus only) — evenodd path creates a true ring
    // with a transparent hole in the middle. No fill bleeds into the neutral zone.
    ...
    p.push(`<path d="${ao} ${ai}" fill="color-mix(in srgb,var(--rsi-hi) 17%,transparent)" fill-rule="evenodd" stroke="none"/>`);

    // Light core (RSI ≤ 30) — transparent middle is "no man's land"; blue on extreme
    const r30  = 0.3 * R;   // 16.2
    const coreFill = hasExtreme ? 'color-mix(in srgb,var(--warn) 30%,transparent)' : 'color-mix(in srgb,var(--rsi-lo) 26%,transparent)';
    ...
    // Dots — blue for extreme oversold (RSI ≤ 10), light for oversold (≤ 30), dark for overbought (≥ 70)
    axes.forEach(ax => {
      const rsi = rsiMap[ax.tf];
      if (rsi === null || rsi === undefined) return;
      const [dx, dy] = pt(ax.angle, Math.max(3, (rsi / 100) * R));
      const dotCol = rsi >= 70 ? 'var(--rsi-hi)' : rsi <= 10 ? 'var(--warn)' : rsi <= 30 ? 'var(--rsi-lo)' : accent;
  ...
  function _rsiLabel(rsi) {
    ...
    const col = rsi >= 70 ? 'var(--rsi-hi-ink)' : rsi <= 30 ? 'var(--rsi-lo-ink)' : 'var(--muted)';
    return `<span style="color:${col};font-weight:600">${rsi.toFixed(0)}</span>`;
  }

  function _chgSpan(v) {
    if (v === null || v === undefined) return '<span class="muted">—</span>';
    const col = v >= 0 ? 'var(--good-ink)' : 'var(--bad)';
...
${window.location.origin}`;
  ...
  function _sigPanelHTML() {
    ...
    return `
      ...
      <div class="radar-sig-row">
        <span style="min-width:72px">Server</span>
      ...
      <div class="radar-sig-note">
        ...
        ${_serverMode()
          ...
          : 'Checks every 5 min while the dashboard is open in any tab · re-alerts max once per 6h. Turn on to run it off the browser.'}
        Email needs SMTP_USER/SMTP_PASS set on Railway.
      </div>
      <div class="radar-sig-row"><span id="radarSigStatus" class="muted" style="font-size:.7rem"></span></div>`;
  }
  ...
  function _tradeScores(rsiMap) {
    return Object.entries(SCORE_CLUSTERS).map(([key, { tfs, label }]) => {
      ...
      let grade, col;
      if      (avg < 30 && strong) { grade = 'A'; col = 'var(--rsi-lo-ink)'; }
      else if (avg < 38)           { grade = 'B'; col = 'var(--rsi-lo-ink)'; }
      else if (avg > 70 && strong) { grade = 'A'; col = 'var(--rsi-hi-ink)'; }
      else if (avg > 62)           { grade = 'B'; col = 'var(--rsi-hi-ink)'; }
      /* A and B share the ink so both stay legible; the badge separates them
         by weight and border strength instead of by washing the colour out. */
  ...
  function _scoreBadges(rsiMap) {
    ...
    return `<div class="radar-scores">${scores.map(s =>
      `<span class="radar-score-badge" data-grade="${s.grade}" style="border-color:color-mix(in srgb,${s.col} ${s.grade === 'A' ? 62 : 30}%,transparent);color:${s.col}" title="${s.label}">
  ...
  function _cardHTML(card) {
    ...
    const footer = isUsdtD
      ? `<div class="radar-footer">
          ...
          <span class="muted" style="font-size:.66rem">Tether dominance · macro risk gauge</span>
          ${_usdtdSrcChip()}
        </div>`
      : `<div class="radar-footer">
  ...
  function _buildGuideHTML() {
    ...
    <div class="radar-guide card" style="margin-top:24px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px 28px;margin-top:12px;font-size:.75rem;line-height:1.5">
        <div>
          <strong>Spoke = timeframe</strong> — 5m · 15m · 1h · 4h · D · W · M. Dot distance from center = RSI value (0 = center, 100 = edge).
        </div>
        <div>
          <strong style="color:var(--rsi-lo-ink)">Green core</strong> RSI ≤ 30 (oversold). &nbsp;<strong style="color:var(--rsi-hi-ink)">Red ring</strong> RSI ≥ 70 (overbought). &nbsp;Middle = no man's land. Dots <strong style="color:var(--warn)">flare</strong> at RSI ≤ 10 (extreme).
        </div>
        <div>
          <strong>USDT.D (top-left)</strong> — RSI of USDT mcap ÷ BTC price. Rising = risk-off / stablecoin rotation. Source chip: <strong style="color:var(--good-ink)">live</strong> / <strong style="color:var(--warn)">cached</strong> / <strong style="color:var(--warn)">rebuilt</strong>.
        </div>
        <div>
          <strong>Grade badges</strong> — <strong style="color:var(--rsi-lo-ink)">A</strong> avg RSI &lt;30 or &gt;70, TFs agree → full size. <strong style="color:var(--text-2)">B</strong> &lt;38 or &gt;62 → half. <strong style="color:var(--muted)">C</strong> &lt;45 or &gt;55 → wait. <strong style="color:var(--muted)">D</strong> 45–55 → skip.<br>
          <span style="font-size:.66rem;color:var(--muted)"><strong>S</strong> 5m·15m·1h &nbsp;·&nbsp; <strong>SW</strong> 1h·4h·D &nbsp;·&nbsp; <strong>L</strong> D·W·M</span>
        </div>
        <div>
          <strong style="color:var(--warn)">★ + Swing signals</strong> — star a coin, and when its <strong>1h · 4h · D</strong> RSI are ALL oversold (SW Long) or ALL overbought (SW Short) you get a Telegram / email alert. Configure in <strong>Signals</strong>; Telegram creds live in Pro Tools → Telegram.
        </div>
      </div>

      <div style="margin-top:14px">
        <div style="font-size:.7rem;font-weight:700;margin-bottom:10px;color:var(--text)">Visual examples</div>
        <div class="radar-guide-examples">
          <div class="radar-guide-ex">
            ${exScalpLong}
            <div class="radar-ex-badge" style="color:var(--rsi-lo-ink)">S A ▲ Long</div>
            <div class="radar-ex-label">Scalp Long<br><span>5m·15m·1h in green core</span><br><span>Entry: 5m–15m · Hold: 1–4h</span></div>
          </div>
          <div class="radar-guide-ex">
            ${exScalpShort}
            <div class="radar-ex-badge" style="color:var(--rsi-hi-ink)">S A ▼ Short</div>
            <div class="radar-ex-label">Scalp Short<br><span>5m·15m·1h in red ring</span><br><span>Entry: 5m–15m · Hold: 1–4h</span></div>
          </div>
          <div class="radar-guide-ex">
            ${exSwingLong}
            <div class="radar-ex-badge" style="color:var(--rsi-lo-ink)">SW A ▲ Long</div>
            <div class="radar-ex-label">Swing Long<br><span>1h·4h·D in green core</span><br><span>Entry: 1h–4h · Hold: 1–5D</span></div>
          </div>
          <div class="radar-guide-ex">
            ${exSwingShort}
            <div class="radar-ex-badge" style="color:var(--rsi-hi-ink)">SW A ▼ Short</div>
            <div class="radar-ex-label">Swing Short<br><span>1h·4h·D in red ring</span><br><span>Entry: 1h–4h · Hold: 1–5D</span></div>
          </div>
        </div>
      </div>

      <div class="muted" style="font-size:.66rem;margin-top:10px">4h/W spokes use subsampling — directionally accurate, ≤3h/6d boundary drift. Auto-refresh every 5 min (toggle in bar).</div>
    </div>`;
  }

  /* ── Main render ─────────────────────────────────────────── */
  function render(mountId) {
    _mountId = mountId || 'view';
    ...
    root.innerHTML = `
      <div class="radar-bar">
        ...
        <div class="radar-bar-right">
          ...
          <button class="btn-ghost" id="radarAddBtn" style="font-size:.75rem">＋ Add</button>
          <button class="btn-ghost radar-auto-btn${_autoOn() ? ' active' : ''}" id="radarAutoBtn" style="font-size:.75rem"
            title="Auto-refresh every 5 minutes" onclick="CryptoRadarTab._toggleAuto()">Auto 5m ${_autoOn() ? '✓' : '✗'}</button>
          <button class="btn-ghost radar-sig-btn${_sigOn() ? ' active' : ''}" id="radarSigBtn" style="font-size:.75rem"
      ...
      <div class="radar-substatus">
        <span id="radarAddStatus" class="muted" style="font-size:.7rem"></span>
...
/* ── Page wrapper for the Dashboard 3 router ──────────────────────────
   CryptoRadarTab above is the full radar carried over from the old
   dashboard, untouched: one card per coin, seven timeframes, the USDT.D
   card pinned first, editable watchlist, ★ favourites and swing alerts. */
const RadarPage = {
  render: () => CryptoRadarTab.render('view'),
  title:  'Radar',
  sub:    () => 'One card per coin · seven timeframes · USDT.D first',
};
