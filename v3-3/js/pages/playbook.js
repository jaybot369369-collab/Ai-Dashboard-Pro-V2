...
const PlaybookTab = (() => {
  ...
  /* Charter §3: setup verdicts need ≥20 closed trades. Below that a setup is
     "collecting data" — never given a verdict off a tiny sample. */
  const VERDICT_MIN_N = 20;
  function wrBadge(wr, n) {
    if (wr === null) return { color: 'var(--text-3)', label: '—' };
    if ((n || 0) < VERDICT_MIN_N)
      return { color: 'var(--text-3)', label: `${n} of ${VERDICT_MIN_N} trades · ${wr.toFixed(0)}% won so far` };
    if (wr >= 75)   return { color: 'var(--good-ink)', label: wr.toFixed(0) + '%' };
    if (wr >= 60)   return { color: 'var(--warn)', label: wr.toFixed(0) + '%' };
    if (wr >= 50)   return { color: 'color-mix(in srgb,var(--warn) 60%,var(--bad))', label: wr.toFixed(0) + '%' };
    return { color: 'var(--bad)', label: wr.toFixed(0) + '%' };
  }
    ...
    fvg: [
      {
        ...
        zones:   [{ yHi:55, yLo:53, color:'var(--good)', opacity:0.22, label:'FVG' }],
        lines:   [{ y:54, color:'var(--good)', dash:true, label:'Entry' }],
        markers: [{ type:'arrow_up', candle:10, y:54, color:'var(--good)' }],
      },
      {
        ...
        zones:   [{ yHi:69, yLo:66, color:'var(--bad)', opacity:0.22, label:'FVG' }],
        lines:   [{ y:67, color:'var(--bad)', dash:true, label:'Entry' }],
        markers: [{ type:'arrow_down', candle:10, y:70, color:'var(--bad)' }],
      },
      {
        ...
        zones:   [{ yHi:67, yLo:65, color:'var(--warn)', opacity:0.25, label:'IFVG' }],
        lines:   [{ y:66, color:'var(--warn)', dash:true }],
        markers: [{ type:'arrow_down', candle:12, y:69, color:'var(--bad)' }],
      },
      {
        ...
        zones:   [{ yHi:63, yLo:57, color:'var(--good)', opacity:0.15, label:'FVG' }],
        lines:   [
          { y:60, color:'var(--good)', dash:true, label:'50%' },
        ],
        markers: [{ type:'arrow_up', candle:9, y:60, color:'var(--good)' }],
      },
      {
        ...
        zones: [
          { yHi:61, yLo:59, color:'var(--good)', opacity:0.22, label:'FVG' },
          { yHi:59, yLo:55, color:'var(--text-3)', opacity:0.20, label:'OB'  },
        ],
        markers: [{ type:'arrow_up', candle:10, y:57, color:'var(--good)' }],
      },
    ],
    ...
    turtle: [
      {
        ...
        lines:   [{ y:70, color:'var(--warn)', dash:true, label:'Prev High' }],
        markers: [
          { type:'tag',        candle:10, y:75.5, color:'var(--bad)', text:'SWEEP' },
          { type:'arrow_down', candle:11, y:65,   color:'var(--bad)' },
        ],
      },
      {
        ...
        lines:   [{ y:64, color:'var(--warn)', dash:true, label:'Prev Low' }],
        markers: [
          { type:'tag',       candle:10, y:59.5, color:'var(--good)', text:'SWEEP' },
          { type:'arrow_up',  candle:11, y:63,   color:'var(--good)' },
        ],
      },
      {
        ...
        lines:   [{ y:68, color:'var(--warn)', dash:true, label:'Equal Highs' }],
        markers: [
          { type:'tag',        candle:10, y:74.5, color:'var(--bad)', text:'BSL SWEPT' },
          { type:'arrow_down', candle:11, y:64,   color:'var(--bad)' },
        ],
      },
      {
        ...
        zones:   [{ yHi:66, yLo:62, color:'var(--text-3)', opacity:0.15, label:'Asia Range' }],
        markers: [
          { type:'tag',        candle:7, y:75.5, color:'var(--bad)', text:'FALSE BRK' },
          { type:'arrow_down', candle:8, y:64,   color:'var(--bad)' },
        ],
      },
      {
        ...
        zones:   [{ yHi:60, yLo:57, color:'var(--good)', opacity:0.22, label:'FVG' }],
        lines:   [{ y:63.5, color:'var(--warn)', dash:true, label:'Equal Lows' }],
        markers: [
          { type:'tag',      candle:8, y:56.5, color:'var(--good)', text:'STOP HUNT' },
          { type:'arrow_up', candle:9, y:62,   color:'var(--good)' },
        ],
      },
    ],
    ...
    silver_bullet: [
      {
        ...
        zones:   [{ yHi:67, yLo:64, color:'var(--good)', opacity:0.22, label:'FVG' }],
        lines:   [{ y:60, color:'var(--warn)', dash:true, label:'Session Low' }],
        markers: [{ type:'arrow_up', candle:10, y:66, color:'var(--good)' }],
      },
      {
        ...
        zones:   [{ yHi:71, yLo:68, color:'var(--bad)', opacity:0.22, label:'FVG' }],
        lines:   [{ y:74, color:'var(--warn)', dash:true, label:'Session High' }],
        markers: [{ type:'arrow_down', candle:9, y:73, color:'var(--bad)' }],
      },
      {
        ...
        zones:   [{ yHi:67, yLo:64, color:'var(--good)', opacity:0.22, label:'FVG' }],
        lines:   [{ y:62, color:'var(--warn)', dash:true, label:'Asia Low' }],
        markers: [{ type:'arrow_up', candle:10, y:67, color:'var(--good)' }],
      },
      {
        ...
        zones:   [{ yHi:71, yLo:68, color:'var(--bad)', opacity:0.22, label:'FVG' }],
        lines:   [{ y:74, color:'var(--warn)', dash:true, label:'Noon High' }],
        markers: [{ type:'arrow_down', candle:10, y:73, color:'var(--bad)' }],
      },
      {
        ...
        zones:   [{ yHi:74, yLo:65, color:'var(--good)', opacity:0.12, label:'FVG (unfilled)' }],
        lines:   [{ y:61, color:'var(--warn)', dash:true, label:'SSL' }],
        markers: [
          { type:'tag',      candle:3, y:79, color:'var(--good)', text:'DISPLACEMENT' },
          { type:'arrow_up', candle:4, y:74, color:'var(--good)' },
        ],
      },
    ],
    ...
    order_block: [
      {
        ...
        context: 'The last bearish candle before a strong up-move becomes the Order Block. When price returns to that candle\'s body, institutions re-enter long.',
        candles: [
        ...
        zones:   [{ yHi:65, yLo:61, color:'var(--text-3)', opacity:0.22, label:'Bull OB' }],
        markers: [{ type:'arrow_up', candle:11, y:62, color:'var(--good)' }],
      },
      {
        title: 'Bearish OB — Last Bullish Candle Before Impulse',
        context: 'The final bullish candle before a sharp sell-off defines the Order Block. On the retrace back into that body, smart money shorts.',
        candles: [
        ...
        zones:   [{ yHi:73, yLo:67, color:'var(--bad)', opacity:0.22, label:'Bear OB' }],
        markers: [{ type:'arrow_down', candle:10, y:73, color:'var(--bad)' }],
      },
      {
        ...
        zones: [
          { yHi:64, yLo:59, color:'var(--good)', opacity:0.18, label:'FVG' },
          { yHi:59, yLo:56, color:'var(--text-3)', opacity:0.22, label:'OB'  },
        ],
        markers: [{ type:'arrow_up', candle:10, y:59, color:'var(--good)' }],
      },
      {
        ...
        zones:   [{ yHi:62, yLo:59, color:'var(--text-3)', opacity:0.22, label:'Bull OB' }],
        lines:   [{ y:60.5, color:'var(--text-3)', dash:true, label:'50%' }],
        markers: [{ type:'arrow_up', candle:10, y:63, color:'var(--good)' }],
      },
      {
        ...
        zones:   [{ yHi:64, yLo:59, color:'var(--text-3)', opacity:0.22, label:'Bull OB' }],
        lines:   [{ y:70, color:'var(--warn)', dash:true, label:'MSS' }],
        markers: [
          { type:'tag',      candle:9,  y:77, color:'var(--good)', text:'MSS BREAK' },
          { type:'arrow_up', candle:12, y:68, color:'var(--good)' },
        ],
      },
    ],
    ...
    breaker: [
      {
        ...
        zones:   [{ yHi:74, yLo:70, color:'var(--good)', opacity:0.22, label:'Breaker' }],
        lines:   [{ y:74, color:'var(--good)', dash:true }],
        markers: [
          { type:'tag',      candle:5,  y:79, color:'var(--good)', text:'OB BROKEN' },
          { type:'arrow_up', candle:10, y:70, color:'var(--good)' },
        ],
      },
      {
        ...
        zones:   [{ yHi:67, yLo:63, color:'var(--bad)', opacity:0.22, label:'Breaker' }],
        lines:   [{ y:63, color:'var(--bad)', dash:true }],
        markers: [
          { type:'tag',        candle:5,  y:55, color:'var(--bad)', text:'OB BROKEN' },
          { type:'arrow_down', candle:10, y:66, color:'var(--bad)' },
        ],
      },
      {
        ...
        zones: [
          { yHi:73, yLo:70, color:'var(--warn)', opacity:0.20, label:'FVG'     },
          { yHi:70, yLo:66, color:'var(--bad)', opacity:0.18, label:'Breaker' },
        ],
        markers: [{ type:'arrow_down', candle:10, y:73, color:'var(--bad)' }],
      },
      {
        ...
        zones:   [{ yHi:73, yLo:70, color:'var(--good)', opacity:0.20, label:'Breaker' }],
        markers: [
          { type:'arrow_up', candle:5,  y:71, color:'var(--good)' },
          { type:'arrow_up', candle:8,  y:71, color:'var(--good)' },
          { type:'arrow_up', candle:10, y:71, color:'var(--good)' },
        ],
      },
      {
        ...
        zones:   [{ yHi:75, yLo:72, color:'var(--good)', opacity:0.22, label:'Breaker' }],
        markers: [
          { type:'tag',      candle:4,  y:84, color:'var(--good)', text:'DISPLACEMENT' },
          { type:'arrow_up', candle:11, y:72, color:'var(--good)' },
        ],
      },
    ],
    ...
    cisd: [
      {
        ...
        lines:   [{ y:73, color:'var(--good)', dash:true, label:'Swing High' }],
        markers: [
          { type:'tag',      candle:7, y:79, color:'var(--good)', text:'CISD' },
          { type:'arrow_up', candle:10, y:72, color:'var(--good)' },
        ],
      },
      {
        ...
        lines:   [{ y:65, color:'var(--bad)', dash:true, label:'Swing Low' }],
        markers: [
          { type:'tag',        candle:7,  y:56, color:'var(--bad)', text:'CISD' },
          { type:'arrow_down', candle:10, y:65, color:'var(--bad)' },
        ],
      },
      {
        ...
        zones:   [{ yHi:76, yLo:62, color:'var(--text-3)', opacity:0.15, label:'OB' }],
        lines:   [{ y:67, color:'var(--good)', dash:true, label:'Prior Swing H' }],
        markers: [
          { type:'tag',      candle:3,  y:77, color:'var(--good)', text:'CISD' },
          { type:'arrow_up', candle:9,  y:68, color:'var(--good)' },
        ],
      },
      {
        ...
        lines:   [
          { y:68, color:'var(--good)', dash:true, label:'LTF Swing H' },
          { y:60, color:'var(--text-3)', dash:true, label:'HTF Support' },
        ],
        markers: [
          { type:'tag',      candle:6,  y:73, color:'var(--good)', text:'CISD' },
          { type:'arrow_up', candle:10, y:68, color:'var(--good)' },
        ],
      },
      {
        ...
        lines:   [{ y:70, color:'var(--warn)', dash:true, label:'Swing High' }],
        markers: [
          { type:'tag',        candle:5, y:74.5, color:'var(--bad)', text:'FAILED CISD' },
          { type:'arrow_down', candle:6, y:66,   color:'var(--bad)' },
        ],
      },
    ],
    ...
    ote: [
      {
        ...
        zones:   [{ yHi:69, yLo:64, color:'var(--good)', opacity:0.18, label:'62–79% OTE' }],
        lines:   [{ y:66.5, color:'var(--good)', dash:true, label:'62%' }],
        markers: [{ type:'arrow_up', candle:8, y:65, color:'var(--good)' }],
      },
      {
        ...
        zones:   [{ yHi:73, yLo:68, color:'var(--bad)', opacity:0.18, label:'62–79% OTE' }],
        lines:   [{ y:70, color:'var(--bad)', dash:true, label:'62%' }],
        markers: [{ type:'arrow_down', candle:8, y:72, color:'var(--bad)' }],
      },
      {
        ...
        zones:   [{ yHi:71, yLo:64, color:'var(--good)', opacity:0.18, label:'OTE 62–79%' }],
        lines:   [
          { y:71, color:'var(--good)', dash:false },
          { y:64, color:'var(--good)', dash:true, label:'79%' },
        ],
        markers: [
          { type:'tag',      candle:8, y:63.5, color:'var(--good)', text:'SWEEP' },
          { type:'arrow_up', candle:9, y:64, color:'var(--good)' },
        ],
      },
      {
        ...
        zones:   [{ yHi:76, yLo:71, color:'var(--good)', opacity:0.18, label:'OTE' }],
        lines:   [{ y:69, color:'var(--warn)', dash:true, label:'MSS Level' }],
        markers: [
          { type:'tag',      candle:3,  y:79, color:'var(--good)', text:'MSS' },
          { type:'arrow_up', candle:9,  y:71, color:'var(--good)' },
        ],
      },
      {
        ...
        zones: [
          { yHi:72, yLo:68, color:'var(--good)', opacity:0.22, label:'FVG' },
          { yHi:74, yLo:67, color:'var(--good)', opacity:0.10, label:'OTE' },
        ],
        lines:   [{ y:70, color:'var(--good)', dash:true, label:'62%' }],
        markers: [{ type:'arrow_up', candle:9, y:67, color:'var(--good)' }],
      },
    ],
    ...
    sweep: [
      {
        ...
        lines:   [{ y:70, color:'var(--warn)', dash:true, label:'Equal Highs' }],
        markers: [
          { type:'tag',        candle:7, y:76.5, color:'var(--bad)', text:'BSL TAKEN' },
          { type:'arrow_down', candle:8, y:65,   color:'var(--bad)' },
        ],
      },
      {
        ...
        lines:   [{ y:66, color:'var(--warn)', dash:true, label:'Equal Lows' }],
        markers: [
          { type:'tag',      candle:7, y:59.5, color:'var(--good)', text:'SSL TAKEN' },
          { type:'arrow_up', candle:8, y:64,   color:'var(--good)' },
        ],
      },
      {
        ...
        lines:   [
          { y:64, color:'var(--good)', dash:true, label:'SSL' },
          { y:73, color:'var(--bad)', dash:true, label:'BSL' },
        ],
        markers: [
          { type:'tag',        candle:1,  y:63,   color:'var(--good)', text:'SSL' },
          { type:'tag',        candle:6,  y:79.5, color:'var(--bad)', text:'BSL' },
          { type:'arrow_down', candle:7,  y:75,   color:'var(--bad)' },
        ],
      },
      {
        ...
        markers: [
          { type:'tag',      candle:7, y:61, color:'var(--good)', text:'SWEEP' },
          { type:'arrow_up', candle:8, y:62, color:'var(--good)' },
        ],
      },
      {
        ...
        lines:   [{ y:67, color:'var(--text-3)', dash:true, label:'50% Equil.' }],
        markers: [
          { type:'tag',      candle:5, y:59, color:'var(--good)', text:'SSL' },
          { type:'arrow_up', candle:6, y:64, color:'var(--good)' },
        ],
      },
    ],
    ...
    asia: [
      {
        ...
        zones:   [{ yHi:66, yLo:62, color:'var(--text-3)', opacity:0.15, label:'Asia Range' }],
        lines:   [{ y:66, color:'var(--text-3)', dash:true, label:'Asia High' }],
        markers: [
          { type:'tag',      candle:6,  y:77, color:'var(--good)', text:'LONDON BRK' },
          { type:'arrow_up', candle:10, y:66, color:'var(--good)' },
        ],
      },
      {
        ...
        zones:   [{ yHi:67, yLo:63, color:'var(--text-3)', opacity:0.15, label:'Asia Range' }],
        lines:   [{ y:63, color:'var(--bad)', dash:true, label:'Asia Low' }],
        markers: [
          { type:'tag',        candle:6,  y:53, color:'var(--bad)', text:'LONDON BRK' },
          { type:'arrow_down', candle:10, y:65, color:'var(--bad)' },
        ],
      },
      {
        ...
        zones:   [{ yHi:66, yLo:62, color:'var(--text-3)', opacity:0.15, label:'Asia Range' }],
        markers: [
          { type:'tag',        candle:5, y:75.5, color:'var(--bad)', text:'BSL SWEPT' },
          { type:'arrow_down', candle:6, y:64,   color:'var(--bad)' },
        ],
      },
      {
        ...
        zones:   [{ yHi:67, yLo:63, color:'var(--text-3)', opacity:0.15, label:'Asia Range' }],
        markers: [
          { type:'tag',      candle:5, y:58.5, color:'var(--good)', text:'SSL SWEPT' },
          { type:'arrow_up', candle:6, y:59,   color:'var(--good)' },
        ],
      },
      {
        ...
        zones:   [{ yHi:66, yLo:65, color:'var(--text-3)', opacity:0.25, label:'Asia Range' }],
        markers: [
          { type:'tag',      candle:6, y:80.5, color:'var(--good)', text:'EXPANSION' },
          { type:'arrow_up', candle:7, y:65,   color:'var(--good)' },
        ],
      },
    ],
    ...
    amd: [
      {
        ...
        zones: [
          { yHi:66, yLo:63, color:'var(--text-3)', opacity:0.15, label:'A — Accumulation' },
        ],
        markers: [
          { type:'tag', candle:4, y:58.5, color:'var(--warn)', text:'M — Manip.'    },
          { type:'tag', candle:7, y:75,   color:'var(--good)', text:'D — Distrib.'  },
        ],
      },
      {
        ...
        zones: [
          { yHi:66, yLo:63, color:'var(--text-3)', opacity:0.15, label:'A — Asia Range' },
        ],
        markers: [
          { type:'tag',        candle:4, y:74.5, color:'var(--warn)', text:'M — Manip.' },
          { type:'tag',        candle:5, y:58,   color:'var(--bad)', text:'D — Distrib.' },
          { type:'arrow_down', candle:5, y:64,   color:'var(--bad)' },
        ],
      },
      {
        ...
        zones:   [{ yHi:64, yLo:60, color:'var(--text-3)', opacity:0.22, label:'OB (Manip. Low)' }],
        markers: [
          { type:'tag',      candle:4, y:56.5, color:'var(--warn)', text:'MANIPULATION' },
          { type:'arrow_up', candle:9, y:65,   color:'var(--good)' },
        ],
      },
      {
        ...
        zones: [
          { yHi:67, yLo:64, color:'var(--text-3)', opacity:0.12, label:'Accumulation' },
        ],
        lines: [{ y:61, color:'var(--warn)', dash:true, label:'Manip. Low' }],
        markers: [
          { type:'arrow_up', candle:7, y:64, color:'var(--good)' },
          { type:'tag',      candle:9, y:75, color:'var(--good)', text:'DISTRIBUTION' },
        ],
      },
      {
        ...
        zones: [
          { yHi:68, yLo:63, color:'var(--text-3)', opacity:0.15, label:'Mon–Tue (A)' },
        ],
        markers: [
          { type:'tag',      candle:2, y:60.5, color:'var(--warn)', text:'Wed (M)' },
          { type:'tag',      candle:4, y:74,   color:'var(--good)', text:'Thu–Fri (D)' },
          { type:'arrow_up', candle:3, y:61.5, color:'var(--good)' },
        ],
      },
    ],
    ...
    killzone: [
      {
        ...
        zones:   [{ yHi:65, yLo:61, color:'var(--text-3)', opacity:0.22, label:'Bull OB' }],
        lines:   [{ y:63, color:'var(--warn)', dash:true, label:'Asia Low' }],
        markers: [{ type:'arrow_up', candle:9, y:67, color:'var(--good)' }],
      },
      {
        ...
        zones:   [{ yHi:70, yLo:67, color:'var(--good)', opacity:0.18, label:'FVG' }],
        lines:   [{ y:68, color:'var(--warn)', dash:true, label:'London Low' }],
        markers: [
          { type:'tag',      candle:5, y:63.5, color:'var(--good)', text:'NY OPEN' },
          { type:'arrow_up', candle:6, y:67,   color:'var(--good)' },
        ],
      },
      {
        ...
        zones:   [{ yHi:76, yLo:72, color:'var(--bad)', opacity:0.22, label:'Bear OB' }],
        lines:   [{ y:72, color:'var(--warn)', dash:true, label:'2pm Level' }],
        markers: [
          { type:'tag',        candle:5, y:77.5, color:'var(--bad)', text:'2PM KZ' },
          { type:'arrow_down', candle:6, y:76,   color:'var(--bad)' },
        ],
      },
      {
        ...
        zones:   [{ yHi:77, yLo:73, color:'var(--bad)', opacity:0.22, label:'London High OB' }],
        markers: [
          { type:'tag',        candle:4, y:78.5, color:'var(--bad)', text:'LDN CLOSE KZ' },
          { type:'arrow_down', candle:5, y:75,   color:'var(--bad)' },
        ],
      },
      {
        ...
        zones: [
          { yHi:74, yLo:70, color:'var(--bad)', opacity:0.20, label:'Asia Bear OB' },
          { yHi:67, yLo:64, color:'var(--text-3)', opacity:0.12, label:'Asia Range Low' },
        ],
        markers: [
          { type:'tag',        candle:6,  y:57,   color:'var(--good)', text:'ASIA SSL' },
          { type:'arrow_down', candle:10, y:74,   color:'var(--bad)' },
        ],
      },
    ],
    ...
    continuation: [
      {
        ...
        zones:   [{ yHi:62, yLo:59, color:'var(--text-3)', opacity:0.22, label:'Bull OB' }],
        markers: [
          { type:'tag',      candle:2,  y:75, color:'var(--good)', text:'1st entry'  },
          { type:'arrow_up', candle:8,  y:67, color:'var(--good)' },
        ],
      },
      {
        ...
        zones:   [{ yHi:66, yLo:60, color:'var(--good)', opacity:0.18, label:'FVG' }],
        markers: [
          { type:'arrow_up', candle:7, y:67, color:'var(--good)' },
        ],
      },
      {
        ...
        lines:   [{ y:67, color:'var(--good)', dash:true, label:'Prior High → Support' }],
        markers: [
          { type:'tag',      candle:4,  y:74, color:'var(--good)', text:'STRUCTURE BRK' },
          { type:'arrow_up', candle:8,  y:68, color:'var(--good)' },
        ],
      },
      {
        ...
        zones:   [{ yHi:73, yLo:70, color:'var(--text-3)', opacity:0.15, label:'Consolidation' }],
        markers: [
          { type:'tag',      candle:6, y:80, color:'var(--good)', text:'BREAKOUT' },
          { type:'arrow_up', candle:8, y:73, color:'var(--good)' },
        ],
      },
      {
        ...
        lines:   [{ y:67, color:'var(--warn)', dash:true, label:'Recent Low' }],
        markers: [
          { type:'tag',      candle:5, y:62.5, color:'var(--good)', text:'SWEEP' },
          { type:'arrow_up', candle:6, y:63,   color:'var(--good)' },
        ],
      },
    ],
  ...
  function makeMiniChart(ex, W, H) {
    ...
    /* --good is the light AREA fill; anything that paints a mark (label,
       hairline, arrow) has to use the readable ink of the same hue. */
    const ink = c => (c === 'var(--good)' ? 'var(--good-ink)' : c);
    ...
    for (const z of (ex.zones || [])) {
      ...
      if (z.label) s += `<text x="${parseFloat(x1) + 4}" y="${(zy1 + 10).toFixed(1)}" font-size="9" fill="${ink(z.color)}" font-family="system-ui,sans-serif" font-weight="700" opacity="1">${z.label}</text>`;
    }
    ...
    for (const l of (ex.lines || [])) {
      ...
      s += `<line x1="0" y1="${ly}" x2="${W}" y2="${ly}" stroke="${ink(l.color)}" stroke-width="1.1"${dash} opacity="0.9"/>`;
      if (l.label) s += `<text x="${W - 4}" y="${(parseFloat(ly) - 2.5).toFixed(1)}" text-anchor="end" font-size="8.5" fill="${ink(l.color)}" font-family="system-ui,sans-serif" font-weight="700">${l.label}</text>`;
    }
    ...
    for (let i = 0; i < n; i++) {
      ...
      const bull = c.c >= c.o;
      const col  = bull ? 'var(--good)' : 'var(--bad)';
    ...
    for (const m of (ex.markers || [])) {
      ...
      if (m.type === 'arrow_up') {
        ...
        s += `<polygon points="${mx.toFixed(1)},${tip.toFixed(1)} ${(mx-5).toFixed(1)},${base.toFixed(1)} ${(mx+5).toFixed(1)},${base.toFixed(1)}" fill="${ink(m.color) || 'var(--good-ink)'}"/>`;
      } else if (m.type === 'arrow_down') {
        const ay = parseFloat(py(m.y));
        const tip = ay - 2, base = ay - 16;
        s += `<polygon points="${mx.toFixed(1)},${tip.toFixed(1)} ${(mx-5).toFixed(1)},${base.toFixed(1)} ${(mx+5).toFixed(1)},${base.toFixed(1)}" fill="${ink(m.color) || 'var(--bad)'}"/>`;
      } else if (m.type === 'tag') {
        const ty  = parseFloat(py(m.y));
        const anc = mx > W / 2 ? 'end' : 'start';
        const tx  = anc === 'end' ? mx - 4 : mx + 4;
        s += `<text x="${tx.toFixed(1)}" y="${(ty + 4).toFixed(1)}" text-anchor="${anc}" font-size="8.5" fill="${ink(m.color) || 'var(--text)'}" font-family="system-ui,sans-serif" font-weight="800">${m.text}</text>`;
      }
    }
    ...
    const cards = found.list.map((ex, idx) => `
      <div style="background:var(--surface,var(--bg-2,#f9fafb));border:1px solid var(--border);border-radius:10px;overflow:hidden;display:flex;flex-direction:column">
        ...
        <div style="padding:10px 13px 12px">
          <div style="font-size:.75rem;font-weight:700;color:var(--text);margin-bottom:4px">${esc(ex.title)}</div>
          <div style="font-size:.72rem;color:var(--text-2);line-height:1.5">${esc(ex.context)}</div>
        </div>
      </div>
    `).join('');
    ...
    modal.innerHTML = `
      <div style="background:var(--bg-card,#fff);border-radius:16px;width:100%;max-width:980px;box-shadow:0 24px 64px rgba(0,0,0,.28);overflow:hidden">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;padding:18px 22px 16px;border-bottom:1px solid var(--border)">
          <div>
            <div style="display:flex;align-items:center;gap:10px">
              ...
              <div>
                <div style="font-size:1.02rem;font-weight:700;color:var(--text)">${esc(found.label)} — Chart Examples</div>
                <div style="font-size:.75rem;color:var(--text-2);margin-top:1px">5 real setup patterns · light = bullish · dark = bearish</div>
              </div>
            </div>
          </div>
          <button
            onclick="document.getElementById('pb_examples_modal')?.remove()"
            style="width:32px;height:32px;border:none;background:var(--surface,var(--bg-2,#f0f0f0));border-radius:50%;font-size:1.08rem;line-height:1;cursor:pointer;color:var(--text-2);flex-shrink:0;display:flex;align-items:center;justify-content:center"
        ...
        <div style="padding:12px 22px 18px;border-top:1px solid var(--border);display:flex;gap:16px;flex-wrap:wrap">
          <div style="display:flex;align-items:center;gap:6px;font-size:.72rem;color:var(--text-2)"><span style="display:inline-block;width:12px;height:10px;background:var(--good);border-radius:2px"></span>Bullish / Long setup</div>
          <div style="display:flex;align-items:center;gap:6px;font-size:.72rem;color:var(--text-2)"><span style="display:inline-block;width:12px;height:10px;background:var(--bad);border-radius:2px"></span>Bearish / Short setup</div>
          <div style="display:flex;align-items:center;gap:6px;font-size:.72rem;color:var(--text-2)"><span style="display:inline-block;width:12px;height:10px;background:var(--warn);border-radius:2px"></span>IFVG or liquidity level</div>
          <div style="display:flex;align-items:center;gap:6px;font-size:.72rem;color:var(--text-2)"><span style="display:inline-block;width:12px;height:10px;background:var(--text-3);border-radius:2px"></span>OB or range zone</div>
          <div style="margin-left:auto;font-size:.72rem;color:var(--text-2)">Click outside to close</div>
        </div>
      </div>
    `;
  ...
  function render() {
    ...
    content.innerHTML = `
      <div class="page-head">
        <div>
          <div class="sub">${sorted.length} setup${sorted.length !== 1 ? 's' : ''} · a setup needs ${VERDICT_MIN_N} trades before it gets called good or bad</div>
        </div>
        <button onclick="PlaybookTab._addSetup()" style="display:flex;align-items:center;gap:6px;padding:9px 18px;background:var(--accent);color:var(--accent-ink);border:none;border-radius:8px;font-size:.88rem;font-weight:600;cursor:pointer;white-space:nowrap"
          onmouseenter="this.style.opacity='.88'" onmouseleave="this.style.opacity='1'">
          <span style="font-size:1.14rem;line-height:1;margin-top:-1px">+</span> New setup
        </button>
      </div>
        ...
        <div id="pb_view_${esc(safeId)}" style="display:flex;flex-direction:column;flex:1">
          <div style="padding:20px 20px 0">
            ...
            <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:12px">
              ...
              <div style="font-size:.82rem;font-weight:700;color:${badge.color};background:${badge.color}22;padding:3px 10px;border-radius:99px;white-space:nowrap;flex-shrink:0">${badge.label}</div>
            </div>

            <p style="font-size:.82rem;color:var(--text-2);line-height:1.55;margin:0 0 12px">${esc(s.description) || '<em>No description</em>'}</p>

            ${extraRows.map(([label, val]) => `
              <div style="margin-bottom:8px;font-size:.75rem;color:var(--text-2)">
                <div style="font-size:.66rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;margin-bottom:3px">${label}</div>
            ...
            ${(s.checklist || []).length > 0 ? `
              <div style="border-top:1px solid var(--border);padding-top:12px;margin-bottom:16px">
                <div style="font-size:.66rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--text-2);margin-bottom:8px">Pre-Trade Checklist</div>
    ...
    return `
      <div style="display:flex;flex-direction:column;gap:10px">
        <div style="display:flex;justify-content:space-between;align-items:center">
          <div style="font-size:.82rem;font-weight:700;color:var(--text)">Edit Setup</div>
          <button class="btn-icon" onclick="PlaybookTab._del('${safeId}')" title="Delete setup" style="color:var(--bad)"></button>
        </div>
...
/* ── Page wrapper for the Dashboard 3 router ──────────────────────────
   Carried over from the old dashboard untouched so nothing is lost. */
const PlaybookPage = {
  render: () => PlaybookTab.render(),
  title:  'Playbook',
  sub:    () => 'Your setups, and what each one has actually made',
};
