...
const MarketIntelTab = (() => {
  ...
  // Push-to-Railway (localhost only) — sends local JSON straight to Railway Volume
  const LS_RAILWAY_URL = 'mi_railway_url';
  const RAILWAY_DEFAULT = 'https://q2-2026-fund-production.up.railway.app';
  ...
  async function load() {
    ...
    for (const url of sources) {
      try {
        ...
        _data = await r.json();
        _err = null;   // a later source succeeded — clear any earlier failure (e.g. /api 404 on localhost)
        return;
      } catch (e) {
        _err = e.message;
        continue;
  ...
  async function render() {
    ...
    const localRunBtn = _miIsLocal
      ...
      : '';
    // Push to Railway — only on localhost. Sends the current local JSON straight
    // to Railway's Volume via POST /api/market_intel/import so the Railway dashboard
    // shows your fresh local data without any API credits or redeploy.
    const pushRailwayBtn = _miIsLocal
      ? `<button class="btn-ghost btn-sm" id="miPushRailway" title="Push your local data to Railway so everyone on the live dashboard sees it — no API credits needed.">📤 Push to Railway</button>`
      : '';
    ...
    // Wire "Push to Railway" button (localhost only)
    const pushEl = document.getElementById('miPushRailway');
    if (pushEl) {
      pushEl.addEventListener('click', () => _pushToRailway());
    }
  }
  ...
  /* ── Push to Railway ────────────────────────────────────
     Takes the market intel data already loaded in memory
     (_data, freshly generated by "Run Locally") and POSTs it
     to POST /api/market_intel/import on Railway.  The Railway
     fund.api writes it to /data/fund_data/market_intel.json
     (the Volume), and every connected dashboard picks it up on
     their next reload — no API credits, no redeploy needed.

     Auth: same ADMIN_API_SECRET used by "Run Now", cached in
     localStorage after first use.
  ───────────────────────────────────────────────────────── */
  async function _pushToRailway() {
    const btn    = document.getElementById('miPushRailway');
    const status = document.getElementById('miLocalStatus');
    if (!btn || !status) return;
    ...
    if (!_data) {
      status.hidden = false;
      status.className = 'mi-fetch-status mi-fetch-fail';
      status.textContent = '✗ Nothing to push — run "🖥️ Run Locally" first to generate fresh data.';
      return;
    }

    const railwayUrl = (localStorage.getItem(LS_RAILWAY_URL) || RAILWAY_DEFAULT).replace(/\/$/, '');
    let secret = (localStorage.getItem(LS_ADMIN_SECRET) || '').trim();
    if (!secret) {
      secret = (prompt(
        'Enter your ADMIN_API_SECRET (the same key used by "🚀 Run Now").\n' +
        'Find it in Railway → your service → Variables.\n' +
        'Saved on your Mac after this — you only enter it once.'
      ) || '').trim();
      if (!secret) return;
      localStorage.setItem(LS_ADMIN_SECRET, secret);
    }
    ...
    status.hidden = false;
    status.className = 'mi-fetch-status mi-fetch-pending';
    status.textContent = `Sending ${Math.round(JSON.stringify(_data).length / 1024)}KB to Railway…`;

    try {
      const r = await fetch(`${railwayUrl}/api/market_intel/import`, {
        method: 'POST',
        headers: {
      ...
      if (r.status === 403) {
        localStorage.removeItem(LS_ADMIN_SECRET);
        throw new Error('wrong admin secret — cleared. Click again to re-enter.');
      }
      if (r.status === 503) {
        throw new Error('Railway has ADMIN_API_SECRET unset — add it in Railway → Variables.');
      }
      if (!r.ok) {
        const j = await r.json().catch(() => ({}));
        throw new Error(j.error || `HTTP ${r.status}`);
      ...
      const j = await r.json();
      localStorage.setItem(LS_RAILWAY_URL, railwayUrl);
      status.className = 'mi-fetch-status mi-fetch-ok';
      status.textContent = `✓ Pushed ${Math.round(j.written_bytes / 1024)}KB to Railway. ` +
        `The live dashboard now shows this data on next reload.`;
    } catch (e) {
      status.className = 'mi-fetch-status mi-fetch-fail';
    ...
    btn.disabled = false;
    btn.textContent = '📤 Push to Railway';
  }
